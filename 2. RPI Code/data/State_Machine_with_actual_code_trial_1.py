...
#3) Recording Motion Mode:
    ...
    def Disconnect():
        dxl_coM_result, dxl_error = packetHandler.write1ByteTxRx(port_num, DXL_ID[1], ADDR_MX_TORQUE_ENABLE, 0)
        print("Robot is disconnected from the phone")
        print("Please connect again or turn off")
        state7 = info[0]
        if state8 == "C":
            Connect()
        else:
    ...
    def calculating_speed(new_pos,old_pos):
        ...
        speed1 = abs(((((new_pos - old_pos)*0.029)/300)/sampling_rate)/0.00185)
        return speed, speed1   
    ...
    #project will be truned on till this code
