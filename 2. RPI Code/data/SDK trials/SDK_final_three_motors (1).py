...
dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(port_num, DXL_ID[1], ADDR_MX_TORQUE_ENABLE, 0)  # TORQUE_DISABLE
...
def readfile():
    ...
    with open(filename, 'r') as csvfile:
        ...
        dxl_speed2, dxl_comm_result2, dxl_error2 = packetHandler.read2ByteTxRx(port_num, 3, 38)
        print("current Speed",dxl_speed2)
...
def check_position_reached(goal_Pos ,goal_Pos1 ,goal_Pos2):
    ...
    while 1:
        ...
        if dxl_comm_result != COMM_SUCCESS:
            ...
            print("this is the goal position-",dxl_present_position,dxl_present_position1,dxl_present_position2)
            if ((check_threshold(goal_Pos, dxl_present_position,1)==False) and (check_threshold(goal_Pos1, dxl_present_position1,2)==False) and (check_threshold(goal_Pos2, dxl_present_position2,3))==False):
                print("this is the goal position-",dxl_present_position,dxl_present_position1,dxl_present_position2)
                break

def check_threshold(goal_Pos, dxl_present_position,ID):
    if ID==2:
       return abs(goal_Pos - dxl_present_position) > 30
    else:
